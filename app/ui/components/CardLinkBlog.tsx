...
export const CardLinkBlog = ({ blog, featured = false }: CardLinkBlogProps) => {
  return (
    <Link
      href={`/blog/${blog.slug}`}
      className="flex flex-col h-full bg-white/[0.02] border border-white/10 rounded-[20px] p-8 transition-all duration-300 hover:-translate-y-1 hover:bg-white/[0.04] hover:border-accent hover:shadow-[0_10px_40px_rgba(0,0,0,0.3)] relative overflow-hidden group no-underline"
    >
      ...
      <h3
        className={`font-bold mb-4 leading-tight text-text-light transition-colors duration-300 group-hover:text-accent flex-grow ${
